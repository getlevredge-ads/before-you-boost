    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 p-6 flex items-center justify-center">
        <div className="max-w-2xl w-full bg-slate-800 border border-slate-700 rounded-lg p-8">
          ...
          <button onClick={() => setPhase('phase1')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition">Start Check →</button>
        </div>
      </div>
    );
  }
            ...
            <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                  <input type="radio" checked={organicSales === 'yes'} onChange={() => setOrganicSales('yes')} className="mr-3 w-4 h-4" />
                  <span className="text-white">Yes, at least once</span>
                </label>
                <label className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                  <input type="radio" checked={organicSales === 'no'} onChange={() => setOrganicSales('no')} className="mr-3 w-4 h-4" />
                ...
                <div className="space-y-2">
                  {['whatsapp', 'phone', 'inperson', 'unsure'].map((opt) => (
                    <label key={opt} className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                      <input type="radio" checked={closingMethod === opt} onChange={() => setClosingMethod(opt)} className="mr-3 w-4 h-4" />
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 space-y-6">
            <div>
              ...
              <div className="space-y-2">
                {['500-2k', '2k-5k', '5k-15k', '15k+'].map((opt) => (
                  <label key={opt} className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                    <input type="radio" checked={priceRange === opt} onChange={() => setPriceRange(opt)} className="mr-3 w-4 h-4" />
            ...
            <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                  <input type="radio" checked={repeatBuy === 'repeat'} onChange={() => setRepeatBuy('repeat')} className="mr-3 w-4 h-4" />
                  <span className="text-white">Repeat (they buy again)</span>
                </label>
                <label className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                  <input type="radio" checked={repeatBuy === 'one-time'} onChange={() => setRepeatBuy('one-time')} className="mr-3 w-4 h-4" />
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Cold', 'Medium', 'Hot'].map((opt) => (
                  <label key={opt} className="flex items-center p-3 border border-slate-600 rounded-lg hover:bg-slate-700 cursor-pointer">
                    <input type="radio" checked={competition === opt} onChange={() => setCompetition(opt)} className="mr-3 w-4 h-4" />
